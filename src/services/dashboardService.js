...
class DashboardService {
  async getOverview(tenantId) {
    ...
    return {
      ...
      delivery: {
        total: (deliveryStats.pending?.count || 0) + (deliveryStats.delivered?.count || 0) + (deliveryStats.failed?.count || 0) + (deliveryStats.returned?.count || 0),
        pending: deliveryStats.pending?.count || 0,
        delivered: deliveryStats.delivered?.count || 0,
        failed: deliveryStats.failed?.count || 0,
        returned: deliveryStats.returned?.count || 0,
        revenue: (deliveryStats.delivered?.revenue || 0) + (deliveryStats.returned?.revenue || 0),
        cost: (deliveryStats.delivered?.cost || 0) + (deliveryStats.returned?.cost || 0),
      },
      recentAssignments,
      recentUnassigned,
    };
